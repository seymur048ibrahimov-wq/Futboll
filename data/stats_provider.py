...
def _team_recent_results(matches: List[dict], team_id: int, team_name: str) -> Tuple[list, list, list]:
    ...
    for m in matches:
        ...
        if gh is None or ga is None:
            continue
        # Use the provider's own match id so repeat fixtures between the same
        # two teams (e.g. the same home/away pairing in a later season) each
        # get their own Elo update. Falling back to date+teams (instead of
        # just team names) still keeps re-fetches idempotent even if "id" is
        # ever missing from the payload.
        fixture_key = m.get("id") or f"{m.get('utcDate', '')}:{home.get('id')}:{away.get('id')}"
        parsed.append((m.get("utcDate", ""), home.get("id"), home.get("name"),
                        away.get("name"), gh, ga, fixture_key))

    parsed.sort(key=lambda x: x[0])

    for _, hid, hname, aname, gh, ga, fixture_key in parsed:
        elo_store.apply_result(fixture_key, hname, aname, gh, ga)

    points, gf_list, ga_list = [], [], []
    for _, hid, hname, aname, gh, ga, fixture_key in parsed[-FORM_LOOKBACK:]:
        is_home = hid == team_id

...
def format_team_report(r):
    lines = [
        f"📊 {r['name']}",
        f"🏆 {r['country']}",
        "",
        "📈 SON FORMA",
        f"{r['form']} (Q=Qalib, B=Bərabər, M=Məğlub)",
        "",
        "⚽ HÜCUM / MÜDAFİƏ",
        f"Ortalama qol vurma: {r['goals_for_avg']}",
        f"Ortalama qol yemə: {r['goals_against_avg']}",
        "",
        f"🧠 Elo reytinqi: {r['elo']}",
    ]
    if r["recent_matches"]:
        lines += ["", "🗓 SON MATÇLAR"]
        lines += [f"• {m}" for m in r["recent_matches"]]
    lines += ["", "🚫 Bukmeyker: MODELƏ DAXİL DEYİL"]
    return "\n".join(lines)

...
def _api_key() -> str:
    """FOOTBALL_DATA_API_KEY is the current name; API_FOOTBALL_KEY is kept as
    a fallback for old .env files. Raises a clear, friendly error instead of
    a bare KeyError if neither is set."""
    key = os.environ.get("FOOTBALL_DATA_API_KEY") or os.environ.get("API_FOOTBALL_KEY")
    if not key:
        raise RuntimeError(
            "FOOTBALL_DATA_API_KEY .env faylında tapılmadı. "
...
async def scan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"⚠️ Skan xətası: {exc}")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """Catches anything not already handled locally so the bot always replies
    with something instead of the user getting silence."""
    logger.exception("Unhandled error while processing update: %s", update, exc_info=context.error)
    if isinstance(update, Update) and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "⚠️ Gözlənilməz xəta baş verdi. Bir az sonra yenidən cəhd edin."
            )
        except Exception:
            pass

...
from data.team_lookup import find_team
from data.stats_provider import build_team_report
from analysis.ranking import format_team_report
...
async def team_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/komanda <ad> — search + analyze one team from the 7 core leagues."""
    query = " ".join(context.args) if context.args else ""
    if not query:
        await update.message.reply_text(
            "İstifadə: /komanda <komandanın adı>\nMəsələn: /komanda Arsenal"
        )
        return
    ...
    if not team:
        await update.message.reply_text(
            f"'{query}' adlı komanda tapılmadı.\n"

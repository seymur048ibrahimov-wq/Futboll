...
async def _send_scan(bot, chat_id: str, api_key: str):
    cards = run_daily_scan(api_key)
    if not cards:
        await bot.send_message(chat_id=chat_id, text="📭 Bu gün siqnal veriləcək uyğun matç tapılmadı.")
        return
...
def build_app() -> Application:
    ...
    hour = int(os.getenv("DAILY_SCAN_HOUR_UTC", "8"))
